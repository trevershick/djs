...
impl DjsError {
    pub fn step_failed<S>(step : S, why: DjsError) -> DjsError where S : Into<String> {
        DjsError::StepFailed(step.into(), Box::new(why))
    }
    pub fn download_failure<S>(url : S, fname: S, e: Box<Error>) -> DjsError where S: Into<String> {
        ...
        match *self {
            DjsError::InvalidConfig(ref opt, ref cur_val, ref notes) =>
                write!(f, "{opt} \"{cur_val}\" is invalid. Hint: {notes}",
                       ...
                       notes = style(notes)),
            DjsError::HttpError(ref re) =>
                write!(f, "An HTTP Error Occured {re}",  re = re ),
            DjsError::HttpRequestFailed(ref url, ref status) =>
                write!(f, "{c} - {u}", c = style(status).red(), u = style(url).green()),
            DjsError::XmlContentError(ref note) =>
                write!(f, "An XML Content error occured : {e}", e = style(note).red()),
            DjsError::DownloadFailure(ref url,ref _dest,ref error) =>
                write!(f, "Downloading {u} failed: {e}", u = style(url).green(), e= error),
            DjsError::StepFailed(ref step,ref nested) =>
                write!(f, "{step} failed due to ----v\n    {nested}", step = step, nested = nested)
        }
    }
}
